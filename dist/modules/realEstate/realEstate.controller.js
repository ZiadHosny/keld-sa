...
export const addRealEstateFromExcelFile = catchAsyncError(async (req, res) => {
    const realEstatesFromExcel = await readExcelFile();
    const realEstates = realEstatesFromExcel.map(realEstate => {
        return {
            region: realEstate[0],
            city: realEstate[1],
...
export const getRealEstates = catchAsyncError(async (req, res) => {
